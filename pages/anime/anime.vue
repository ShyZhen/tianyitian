        ...
        <view class="tui-drop-input-box grid justify-center">
          <tui-dropdown-list :show="dropdownShow" :top="94" :height="400">
            <template v-slot:selectionbox>
              <tui-button size="small" type="white" shape="circle" @tap="dropDownList(-1)">风格选择
...
export default {
  ...
  data() {
    return {
      ...
      tempPath: '',  // 防止叠加效果，每次上传、获取头像更新此处，以及第一次合成的时候
  ...
  onShow() {
    ...
    if (getApp().globalData.rapaintAfterCrop) {
      ...
      this.tempPath = this.avatarPath = getApp().globalData.cropImageFilePath;
    }
  },
  ...
  methods: {
    ...
    changeDate() {
      ...
      if (!that.tempPath) {
        that.tempPath = that.avatarPath
      }

      uniUploadImage(that.tempPath, this.type).then(res => {
        // 此处无法依赖request中的错误处理
    ...
    downloadAvatarAndPaintAll(imageUrl, tempFlag) {
      this.$loading('头像加载中...')
      let that = this;
      uni.downloadFile({
        url: imageUrl,
        ...
        fail: function(e) {
          ...
          uni.showModal({
            ...
            success(res) {
              if (res.confirm) {
                that.downloadAvatarAndPaintAll(imageUrl, tempFlag);
              } else if (res.cancel) {
                console.log('用户点击取消');
              }
            }
      ...
      uni.getUserProfile({
        ...
        success: (res) => {
          if (res.userInfo) {
            ...
            that.downloadAvatarAndPaintAll(userInfo.avatarUrl, true)
    ...
    onChooseAvatar(e) {
      ...
      if (e.detail.avatarUrl) {
        ...
        that.tempPath = userInfo.avatarUrl
      ...
      uni.getImageInfo({
        ...
        success: function(image) {
          ...
          if ((-0.02 <= delta && delta <= 0) || (0 < delta && delta <= 0.02)) {
            that.tempPath = that.avatarPath = tempImagePath;
          } else {
            uni.navigateTo({
